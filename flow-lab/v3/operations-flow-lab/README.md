...
상세 Evidence의 Source of Truth는 Backend 저장소입니다. 주요 근거는 [#176 ChatRoom Outbox](https://github.com/bobfull-project/bobfull-backend/blob/develop/docs/evidence/v3/176-chatroom-outbox/README.md), [#183 Email Outbox](https://github.com/bobfull-project/bobfull-backend/blob/develop/docs/evidence/v3/183-email-outbox/README.md), [#59 Kafka AI Pipeline](https://github.com/bobfull-project/bobfull-backend/blob/develop/docs/evidence/v3/59-kafka-ai-pipeline/README.md), [#66 AI Moderation](https://github.com/bobfull-project/bobfull-backend/blob/develop/docs/evidence/v3/66-ai-moderation/README.md), [#170 Redis Pub/Sub](https://github.com/bobfull-project/bobfull-backend/blob/develop/docs/evidence/v3/170-chat-redis-pubsub/README.md)입니다.
...
Chapter 4의 모든 수치는 [#142 인기 회차 예약 부하 측정](https://github.com/bobfull-project/bobfull-backend/blob/develop/docs/evidence/v3/142-reservation-peak/README.md), [#235 Hot-path 병목 개선](https://github.com/bobfull-project/bobfull-backend/blob/develop/docs/evidence/v3/restaurant-view-hotpath/README.md), [#62 검색 Redis Cache 판단](https://github.com/bobfull-project/bobfull-backend/blob/develop/docs/evidence/v3/62-search-cache/README.md)의 실측값을 그대로 인용한다(`factStatus=measured`). "병목 완전 제거"라고 쓰지 않고 "포화 시작 임계점이 약 40 iter/s에서 약 320 iter/s로 8배 이동했으며, 최고 부하 단계에서는 CPU·HikariCP Pool이 다시 포화된다"고 명시한다. #62(검색 Redis Cache)는 별도 Chapter가 아니라 Chapter 4 학습 상세의 "다른 성능 의사결정" 카드로만 짧게 연결한다.

`#191`(Auto Scaling)은 #191 / PR #276에서 측정을 완료했고, 현재 범위에서는 **Auto Scaling을 도입하지 않고 Active App 2대를 유지**하는 것으로 확정됐다. 이는 `MEASURED_AND_REJECTED` 결정이며 [대표 ADR 0015](../../../adr/0015-no-app-auto-scaling.md)에 요약한다. `#169`(App HA + AWS Redis cross-instance), `#192`(Kafka Async 비교·Consumer scaling·통합 모놀리스 결정), `#274`(Outbox+Async vs Outbox+Kafka 통제 비교 — Kafka 최종 채택 근거), `#258`(messageId Partition Key), `#251`(Rule Fast Path), `#266`(Split Message Rule Context)는 실제 검증이 끝나 각각 Ch3/Ch5/Ch6에 반영됐다. #191은 별도 재생 Chapter로 승격하지 않고 운영 의사결정 Evidence와 ADR에서 연결한다.

Chapter 5·6의 Evidence: [#192 Kafka AI Worker Scaling](https://github.com/bobfull-project/bobfull-backend/blob/develop/docs/evidence/v3/192-ai-worker-scaling/README.md), [#274 Outbox+Async vs Outbox+Kafka Controlled Comparison](https://github.com/bobfull-project/bobfull-backend/blob/develop/docs/evidence/v3/274-outbox-async-vs-kafka/README.md), [#258 Moderation Partition Key](https://github.com/bobfull-project/bobfull-backend/blob/develop/docs/evidence/v3/258-moderation-partition-key/README.md), [#251 AI Moderation Rule Fast Path](https://github.com/bobfull-project/bobfull-backend/blob/develop/docs/evidence/v3/251-ai-moderation-hardening/README.md), [#266 Split Message Moderation](https://github.com/bobfull-project/bobfull-backend/blob/develop/docs/evidence/v3/266-split-message-moderation/README.md), [#169 App HA](https://github.com/bobfull-project/bobfull-backend/blob/develop/docs/evidence/v3/169-app-ha/README.md).
